...
var controller = {
    ...
    saveArchivos: function(req, res) {
        ...
        if (req.files) {
            ...
            archivo.save((err, archivoUpdate) => {
                    if (err) return res.status(500).send({ message: 'Error al subir imagen'});

                    if (!archivoUpdate) return res.status(404).send({ message: 'No se ha podido subir la imagen' });

                    return res.status(200).send({archivo: archivoUpdate});
                });
